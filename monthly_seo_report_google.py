"""
monthly_seo_report_google.py
──────────────────────────────────────────────────────────────────────────
Standalone Google monthly report builder — detect -> review -> build
architecture, scanning the day/Google/<keyword>/ folder layout that
restructure_google_folders.py produces:
        <day>/Google/<keyword folder>/<screenshot files>
  ...
  • Flagging here is COURT / LEGAL / FINANCIAL-CRIME keyword matching
    (see FLAGGED_KEYWORDS) — NOT name-matching, since results are being
    screened for adverse-media mentions, not confirming the person's
    name appears.
  ...
  • Same review-before-build workflow: scan_and_detect() finds every
    screenshot and detects rows only (no images built yet), a review
    tool shows every row so a human can correct anything wrong —
    ...
    the companion review_report_google.py — and only then does
  ...
  • Each row's crop excludes anything past x_right (the real results
    column) — a side panel (e.g. an "AI Answer" box) never gets sliced
    up and re-stacked into the output, since it isn't a real result.

  • A row's row_coords are used EXACTLY as given at build time via
    precomputed_rows/confirmed_flagged — a reviewer's manual edit (crop,
    add, delete, reorder) is never silently discarded by a fresh
    re-detection. Deleted rows are excised via each row's own explicit
    (top, bottom) span, not a shared/rebuilt boundaries list that
    silently assumes every row is still contiguous with its neighbor.

Run it with:
    python monthly_seo_report_google.py
"""
...
def process_google_image(image_path, out_dir, page: int | None = None,
    ...
    if precomputed_rows is not None and precomputed_x_right is not None:
        ...
        # into one shared "boundaries" list, which only works when every
        # row's bottom edge exactly touches the next row's top edge —
        # true for a fresh detection, false the moment a row is deleted
        # in the reviewer (verified on a real screenshot: a naive rebuilt
        # boundaries list silently stitched across the gap, leaving the
        # deleted row's content merged into its neighbor).
    ...
    # here meant their output images collided on the exact same path,
    # so whichever entry got processed last silently overwrote the
    # other's image file on disk — every slide's metadata text stayed
    # correct (that's set independently per slide), but multiple
    # slides ended up displaying the SAME shared image, whatever the
    # last-written one happened to contain.
    ...
    for part_idx, part_size in enumerate(part_sizes):
        ...
        # — a page with a side panel (e.g. an "AI Answer" box) spans a
        # much taller continuous region than any single row, so slicing
        # per-row at full width and re-stacking chops it up into visibly
        # scrambled fragments. The results column doesn't need that
        # panel anyway — it isn't a real search result.
...
def build_csv_rows(csv_results, report_type="Google"):
...
def write_csv_report(report_root, csv_results, month_name, year, report_type="Google"):
    # Filename and the CSV's own content both say which report type this
    # is — "main file.csv" gave no way to tell Google and Explorer
    # reports apart once downloaded, especially with several months'
    # worth sitting in the same downloads folder.
...
def build_pptx(template_path, out_path, month_name, year, week_groups, summary_text):
    ...
    for shp in title_slide.shapes:
        if shp.name.startswith("Subtitle") and shp.has_text_frame:
            shp.text_frame.paragraphs[0].runs[0].text = f"{month_name} {year} — Google"
...
def build_report_from_manifest(report_root, manifest, template_path=None, extra_csv_results=None):
    ...
    csv_path = write_csv_report(report_root, csv_results, manifest["month_name"], manifest["year"], "Google")
    ...
    if template_path and os.path.exists(template_path) and manifest["entries"]:
        ...
        pptx_path = os.path.join(report_root, f"Google_SEO_Report_{month_name}_{year}.pptx")
        summary_text = (
            f"This report covers {month_name} {year}. {total_images} Google search-result "
...
def scan_and_detect(drive_root, year, month_name, month_num, template_path=None):
    ...
    log("=" * 70)
    log("MONTHLY GOOGLE REPORT GENERATOR — detection pass")
    log("=" * 70)
    ...
    report_root = os.path.join(month_folder_path, "Google Report")

    csv_results = defaultdict(lambda: defaultdict(list))
    skipped = []
    jobs = []

    # New, flatter layout: <month>/Google/<day>/<file> — files sit
    # directly in each day folder (no keyword subfolder at all), with
    # the keyword parsed straight from the filename itself, same
    # convention as the Explorer side. This replaced the older
    # <day>/Google/<keyword>/<file> layout specifically so both report
    # types share one consistent shape.
    log("\nScanning for a 'Google' subfolder, then day subfolders inside it ...\n")
    google_dir_name = find_subfolder(month_folder_path, "google")
    if not google_dir_name:
        log("  \u26a0  No 'Google' subfolder found directly inside the month folder \u2014 nothing to scan.")
    else:
        google_path = os.path.join(month_folder_path, google_dir_name)
        day_dirs_parsed = []
        for d in list_dirs(google_path):
        ...
        log(f"Found {len(day_dirs_parsed)} day folder(s) recognized inside 'Google/'.")

        for day_folder_name, day_num, week_num in day_dirs_parsed:
            day_folder_path = os.path.join(google_path, day_folder_name)
...
def main(drive_root, year, month_name, month_num):
    ...
    log(f"\nTip: run review_report_google.py and open '{report_root}' any time to review "
...
class App:
    def __init__(self, root):
        ...
        root.title("Google Monthly SEO Report Generator")
        ...
        tk.Label(header, text="Google Monthly SEO Report Generator", bg=APP_BG, fg=TEXT_FG,
    ...
    def _launch_reviewer(self, report_root):
        """Pops the row-by-row reviewer open immediately after detection
        finishes — no images/CSV/PPTX exist yet at this point. Nothing
        gets built until you review and click "Build Report" in that
        window."""
        try:
            import review_report_google
        except ImportError:
            messagebox.showinfo(
                "Detection complete",
                f"Row detection finished, but review_report_google.py wasn't found alongside "
                f"this script, so the review step can't run. Manifest saved to:\n{report_root}",
            )
            return

        review_win = tk.Toplevel(self.root)
        review_report_google.ReportReviewApp(review_win, report_root=report_root)
